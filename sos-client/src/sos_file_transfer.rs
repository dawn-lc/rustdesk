...
use hbb_common::message_proto::{
    file_action, FileAction, FileResponse, FileTransferBlock, FileTransferReceiveRequest,
...
impl FileTransferHandler {
    pub fn new(tx: mpsc::UnboundedSender<Message>) -> Self {
        Self {
            send_jobs: HashMap::new(),
            recv_jobs: HashMap::new(),
            response_tx: tx,
        }
    }
    ...
    pub fn handle_action(&mut self, action: &FileAction) {
        ...
        if let Some(ref union) = action.union {
            match union {
                ...
                Send(s) => {
                    let mut s2 = s.clone();
                    s2.path = normalize_path(&s2.path);
                    if let Err(e) = self.start_send_job(&s2) {
                        let _ = self.response_tx.send(fs::new_error(s2.id, e, s2.file_num));
                    }
                }
                Receive(r) => {
                    let mut r2 = r.clone();
                    r2.path = normalize_path(&r2.path);
                    if let Err(e) = self.start_receive_job(&r2) {
                        let _ = self.response_tx.send(fs::new_error(r2.id, e, r2.file_num));
                    }
                }
                Cancel(c) => {
                    self.send_jobs.remove(&c.id);
                    self.recv_jobs.remove(&c.id);
                }
                RemoveDir(d) => Self::remove_dir(
                    &normalize_path(&d.path),
                    d.id,
                    d.recursive,
                    &self.response_tx,
                ),
                RemoveFile(f) => Self::remove_file(
                ...
                Rename(r) => Self::rename(
                    r.id,
                    &normalize_path(&r.path),
                    &r.new_name,
                    &self.response_tx,
                ),
                AllFiles(f) => Self::all_files(
                    &normalize_path(&f.path),
                    f.id,
                    f.include_hidden,
                    &self.response_tx,
    ...
    async fn process_send_job(&mut self, id: i32) {
        let job = match self.send_jobs.get_mut(&id) {
            Some(j) => j,
            None => return,
        };
        match job.read().await {
            Err(e) => {
                let _ = self.response_tx.send(fs::new_error(id, e, -1));
                self.send_jobs.remove(&id);
            }
            Ok(Some(blk)) => {
                let _ = self.response_tx.send(fs::new_block(blk));
            }
            Ok(None) if job.job_completed() => {
                let _ = self.response_tx.send(fs::new_done(id, -1));
                self.send_jobs.remove(&id);
            }
            Ok(None) => {}
        }
    ...
    fn read_dir(original_dir: &str, include_hidden: bool, tx: &mpsc::UnboundedSender<Message>) {
        // 标准化路径用于文件系统读取
        ...
        log::info!(
            "[FT] read_dir: original='{}' normalized='{}' path='{:?}'",
            original_dir,
            normalized,
            path
        );
        match fs::read_dir(&path, include_hidden) {
            Ok(mut fd) => {
                // 空路径 = 虚拟根目录 "/"，客户端基于此构造导航路径
                fd.path = if original_dir.is_empty() {
                    "/".to_string()
                } else {
                    original_dir.to_string()
                };
                log::info!(
                    "[FT] read_dir OK: {} entries, response_path='{}'",
                    fd.entries.len(),
                    fd.path
                );
                // 调试：记录前5个条目的名称和类型
                for (i, e) in fd.entries.iter().take(5).enumerate() {
                    log::info!(
                        "[FT] entry[{}]: name='{}' type={:?} is_hidden={} size={}",
                        i,
                        e.name,
                        e.entry_type.enum_value_or_default(),
                        e.is_hidden,
                        e.size
                    );
                }
    ...
    fn remove_dir(path: &str, id: i32, recursive: bool, tx: &mpsc::UnboundedSender<Message>) {
        let r = if recursive {
        ...
        match r {
            Ok(_) => {
                let mut msg = Message::new();
                let mut fr = FileResponse::new();
                let mut d = hbb_common::message_proto::FileDirectory::new();
            ...
            Err(e) => {
                let _ = tx.send(fs::new_error(id, e, -1));
            }
        }
    }

    fn remove_file(path: &str, id: i32, file_num: i32, tx: &mpsc::UnboundedSender<Message>) {
        match std::fs::remove_file(Path::new(path)) {
            ...
            Err(e) => {
                let _ = tx.send(fs::new_error(id, e, file_num));
            }
        }
    }

    fn create_dir(path: &str, id: i32, tx: &mpsc::UnboundedSender<Message>) {
    ...
    fn rename(id: i32, path: &str, new_name: &str, tx: &mpsc::UnboundedSender<Message>) {
        ...
        match std::fs::rename(old, &new) {
            Ok(_) => {
                ...
                let mut fr = FileResponse::new();
                let mut d = hbb_common::message_proto::FileDirectory::new();
                d.path = new.to_string_lossy().to_string();
                fr.set_dir(d);
                msg.set_file_response(fr);
                let _ = tx.send(msg);
            }
    ...
    fn all_files(path: &str, id: i32, include_hidden: bool, tx: &mpsc::UnboundedSender<Message>) {
        match fs::get_recursive_files(path, include_hidden) {
            Ok(files) => {
                let _ = tx.send(fs::new_dir(id, path.to_owned(), files));
            }
            Err(e) => {
                let _ = tx.send(fs::new_error(id, e, -1));
            }
